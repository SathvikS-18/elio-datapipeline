...
> Production-grade ETL pipeline implementing bronze → silver → gold medallion architecture on the Databricks Lakehouse, with analytics queries and commercial recommendations.
...
|-------|---------|--------|
| **Bronze** | Raw ingestion — data as-is with metadata | `raw_customers`, `raw_sales_orders`, `raw_products` |
| **Silver** | Cleaned, typed, normalised (3NF) | `customers`, `orders`, `order_items`, `products` |
| **Gold** | Denormalised star schema for analytics | `dim_customer`, `dim_product`, `dim_date`, `fact_sales` |
...
│   ├── config.py                      ← Centralised config & schemas
│   └── load_bronze.py                 ← PySpark ingestion script
│
├── pipeline/                          ← Parts A2–A4: Transforms
│   ├── 01_create_tables.sql           ← DDL for all medallion layers
│   ├── 02_bronze_to_silver.py         ← Cleaning & normalisation
│   ├── 03_silver_to_gold.py           ← Star schema denormalisation
│   ├── data_model.md                  ← ERD + 3NF rationale
│   └── utils/
│       ├── __init__.py
...
│
├── analysis_answers.md                ← Part B: Insights + recommendation
...
2. **Run the pipeline** in order:
   ...
   3. pipeline/02_bronze_to_silver.py → Cleans and normalises into silver
   4. pipeline/03_silver_to_gold.py   → Builds star schema in gold
   5. sql/analytics_queries.sql       → Run queries against gold layer
   ```

3. **Cluster config** — any single-node or multi-node cluster on DBR 14.3 LTS will work. No additional libraries required.
...
All configuration is centralised in [`ingest/config.py`](ingest/config.py):
...
> **Note:** On Databricks Community Edition (which lacks Unity Catalog), the pipeline uses `hive_metastore` as the catalog. Set `CATALOG_NAME=hive_metastore` in your cluster environment variables.

## 📊 Data Model

### Silver Layer — Normalised (3NF)
...
See [`pipeline/data_model.md`](pipeline/data_model.md) for the full ERD and normalisation rationale.
...
Tests cover:
- **Transform functions** — string standardisation, deduplication, surrogate keys, null handling
...
- A concrete loyalty reactivation campaign recommendation
...
| Decision | Rationale |
|----------|-----------|
| **3NF in Silver, Star in Gold** | Silver = single source of truth (correct, non-redundant). Gold = fast analytics (pre-aggregated, no join sprawl). |
...
| **Modular transform functions** | Reusable, testable, composable. Each table's transform is an isolated function. |
